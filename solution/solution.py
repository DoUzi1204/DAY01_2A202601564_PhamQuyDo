...
def compare_models(prompt: str) -> dict:
    ...
    # Lấy giá của model lớn (mặc định gpt-4o / gemini-2.5-flash)
    pricing = PRICING_PER_1K_TOKENS.get(
        OPENAI_MODEL, PRICING_PER_1K_TOKENS["gpt-4o"]
    )
    # Ước tính số token: 0.75 từ ≈ 1 token
...
def run_assistant(
    ...
    while True:
        ...
        # Đóng gói hàm gọi API có stream=True để truyền vào retry_with_backoff
        def _call():
        ...
        stream = retry_with_backoff(_call)
        reply = ""
        for chunk in stream:
            delta = chunk.choices[0].delta.content or ""
            print(delta, end="", flush=True)
        ...
        history.append({"role": "assistant", "content": reply})
        history = history[-8:]
        # Tính toán thống kê token & chi phí
...
def format_comparison_table(results: list[dict]) -> str:
    ...
    """
    lines = [
        f"{'Prompt':<40} | {'GPT-4o Response':<40} | {'Mini Response':<40} | {'GPT-4o Latency':<14} | {'Mini Latency':<12}",
        "-" * 155,
    ]
    for r in results:
        ...
        lines.append(f"{p:<40} | {g:<40} | {m:<40} | {r['gpt4o_time']:<14.2f} | {r['mini_time']:<12.2f}")
    return "\n".join(lines)
